...
# Captura porcentajes, divisas con símbolo delante/detrás y sufijos de magnitud
NUM_RE = re.compile(
    r"""
    (?<!\w)
    (
        (?:\d+(?:[.,]\d+)?)\s?%                                       # 7.3%
        |
        [€$£]\s*(?:\d{1,3}(?:[.,\s]\d{3})*(?:[.,]\d+)?|\d+(?:[.,]\d+)?) # €2.4 / $1,200.50
        (?:\s?(?:k|m|b|bn|mm))?                                       # €2.4M / €1 bn
        |
        (?:\d{1,3}(?:[.,\s]\d{3})*(?:[.,]\d+)?|\d+(?:[.,]\d+)?)
        (?:\s?(?:k|m|b|bn|mm))?\s?[€$£]                               # 2,4 M€ / 1.2 m$
        |
        (?:\d{1,3}(?:[.,\s]\d{3})*(?:[.,]\d+)?|\d+(?:[.,]\d+)?)
        (?:\s?(?:k|m|b|bn|mm))?                                       # 2.4M / 1,2 m
    )
    (?!\w)
    """,
...
def normalize_number_token(tok: str) -> str:
    ...
    - suprime sufijos (k/m/b/bn/mm) para comparar magnitudes exactas (no escalarlas)
    """
    s = tok.replace("\xa0", " ").strip().lower()
    has_pct = "%" in s
    s = s.replace("€", "").replace("$", "").replace("£", "")
    s = re.sub(r"\b(k|m|b|bn|mm)\b", "", s, flags=re.IGNORECASE).strip()
    core = re.sub(r"[^\d,.\-]", "", s)
    core = core.replace(".", "").replace(",", "").replace(" ", "")
    return (core + "%") if has_pct else core

def numeric_consistency(src: str, tgt: str) -> float:
    s = set(normalize_number_token(x) for x in extract_numbers(src))
    if not s:
        return 1.0
    t = set(normalize_number_token(x) for x in extract_numbers(tgt))
...
def domain_alignment_score(domain: str, text: str) -> float:
    """Heurística con patrones amplios y sinónimos."""
    patterns = {
        "Private Equity": [
            r"\bnav\b", r"\b(ir{1,2}|internal rate of return|tir)\b",
...
# Estructuras para parseo tipado
...
- Consider percentages, currencies (€, $, £), and multipliers k/M/B/bn/mm.
...
async def anumeric_consistency(src: str, tgt: str) -> float:
    ...
    Combinamos: score = max(rule, 0.5*matched_ratio + 0.5*confidence) si ok==false.
    """
    ...
    try:
        ...
        audit: NumericAudit = await llm_parse(prompt, model=getattr(settings, "qa_llm_model", settings.model_review), schema=NumericAudit)
        if audit.ok:
            return 1.0
        llm_score = 0.5 * float(audit.matched_ratio) + 0.5 * float(audit.confidence)
        # mezcla ponderada con la señal de reglas
...
async def adomain_alignment_score(domain: str, text: str) -> float:
    ...
    score = max(rule, (1-w)*rule + w*confidence)
    """
    rule = domain_alignment_score(domain, text)
    try:
        prompt = DOMAIN_AUDIT_PROMPT.format(domain=domain, text=text[:4000])
        audit: DomainAudit = await llm_parse(prompt, model=getattr(settings, "qa_llm_model", settings.model_review), schema=DomainAudit)
        w = float(getattr(settings, "qa_weight_llm", 0.6))
